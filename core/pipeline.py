import luigi
import core.config_file as config
...
class NonMatchImages(luigi.Task):
    dirs = TupleParameter()
...
class TrainingDataGenerator(luigi.Task):
    def output(self):
    ...
    def run(self):
        with self.output().open('w') as out_file:
            ...
            yield MatchImages(dirs, files)
            yield NonMatchImages(dirs, files)
            out_file.write("Status :done")
...
class EncodeData(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget('EncodeData')

    def run(self):
        with self.output().open('w') as out_file:
            print("encoding_data")
            # output_dir = os.getcwd()
            output_dir = '/home/nikhil/data'
            training_dir = output_dir + '/' + config.training_dir + '/'
            testing_dir = output_dir + '/' + config.testing_dir + '/'
            validation_dir = output_dir + '/' + config.validation_dir + '/'
            ...
            yield Encode(training_dir, 'training-images-and-ubyte_13', 'training-labels-and-ubyte_13')
            yield Encode(testing_dir, 'testing-images-and-ubyte_13', 'testing-labels-and-ubyte_13')
            # yield encode(testing_dir, 'testing-labels-and-ubyte_20',  label="label")
            yield Encode(validation_dir, 'validation-images-and-ubyte_13', 'validation-labels-and_ubyte_13')
